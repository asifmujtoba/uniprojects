...
class Oms {
	...
	public function login_check($data){
		...
		if($email == ""){
			$msg['email'] = '<p class="text-center text-danger"><strong>Error! </strong>Field must not be empty!</p>';
		}
		else if(filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
			$msg['email'] = '<p class="text-center text-danger"><strong>Error! </strong>The email address is not valid!</p>';
		}
		if($password == ""){
			$msg['password'] = '<p class="text-center text-danger"><strong>Error! </strong>Field must not be empty!</p>';
		}
		if(!empty($msg)){
			return $msg;
		}
		if($result){
			...
			Session::set("admin", $result-> user_role);
			Session::set("loginmsg", '<p class="text-success"><strong>Success! </strong>You are Login.</p>');
			header("Location: dashboard.php");
		}
		else{
			$msg["error"] = '<p class=" text-center text-danger"><strong>Error! </strong>Data Not Found!</p>';
			return $msg;
		}
	}
	...
	public function save_employee($data){
		...
		$picture =$file_f_name.time().'.'.$file_ext;
		move_uploaded_file($file_temp, $folder.$picture);
	...
	public function alert($message, $type){
		$div = '<div class="alert  alert-dismissible fade show ';
		$class = "";

		if($type == 'success') {$class = 'alert-success';}
		elseif ($type == 'warning'){$class = 'alert-warning';}
		elseif($type == 'info'){$class = 'alert-info' ;}
		elseif($type == 'error'){ $class = 'alert-danger';}
		else{ $class = 'alert-primary';}

		$div2 = '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div></div>';
		$alert = $div .$class .'" role="alert"> <div class="alert-message">'. $message .$div2;
	...
	public function save_task($data){
		...
		if($end_date == ""){
			$msg['end_date'] = $this->alert('<strong>Error! </strong>End Date must not be empty!','success');
		}
		if(!empty($msg)){
			return $msg;
		}
		...
		$result = $query->execute();
		if($result){
			$msg['su'] = $this->alert('<strong>Success! </strong>New Task Assigned.', 'success');
		}
		else{
			$msg['su'] = $this->alert('<strong>Error! </strong>Data Not Insert!</p>', 'error');
		}
		if(!empty($msg)){
			return $msg;
		}
	...
	public function view_my_task($id){
		$sql = "SELECT employee.name, task.* FROM employee INNER JOIN task ON employee.id=task.employee_id WHERE employee_id =$id";
		$query = $this->db->conn->prepare($sql);
		$query -> execute();
		$result = $query->fetchAll();
		return $result;
	...
	public function count_pending_task($id){
		$sql = "SELECT * FROM task  WHERE status='1' && employee_id=$id ORDER BY id DESC LIMIT 0, 3 ";
		$query = $this->db->conn->prepare($sql);
		$query -> execute();
		$result = $query->fetchAll();
	...
	public function count_ongoing_task($id){
		$sql = "SELECT * FROM task  WHERE status='2' && employee_id=$id ORDER BY id DESC LIMIT 0, 3 ";
		$query = $this->db->conn->prepare($sql);
		$query -> execute();
		$result = $query->fetchAll();
		$count_result = count($result);
	...
	public function count_all_task(){
		$sql = "SELECT * FROM task  WHERE status!='3' ORDER BY id DESC LIMIT 0, 3 ";
		$query = $this->db->conn->prepare($sql);
		$query -> execute();
		$result = $query->fetchAll();
		$count_result = count($result);
